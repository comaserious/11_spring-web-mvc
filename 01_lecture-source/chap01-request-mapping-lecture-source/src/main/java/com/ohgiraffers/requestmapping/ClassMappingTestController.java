...
*  내용을 작성하지 않아도 된다
*  이떄 와일드카드(*) 를 이용해서 포괄적인 url 패턴을 사용할수 있다
*  */
public class ClassMappingTestController {
    ...
    public String modifyAndDelete(Model model){
        ...
        model.addAttribute("message", "POST 방식의 주문 정보 수정과 주문 정보 삭제 공통 처리용 메소드 호출됨....");

        return "mappingResult";
    }

    /*필기.
    *  @PathVariable 어노테이션을 이용해 요청 path 로 부터 변수를 받아 올 수 있다
    *  path variable 로 전달되는 {변수명} 은 반드시 매개변수 명과 동일 해야 한다
    *  만약 동일하지 않으면 @PathVariable("이름") 을 설정해주어야 한다*/

    @GetMapping("/detail/{orderNo}/some")
